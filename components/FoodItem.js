    ...
    <>
      <div className="mt-5 single flex flex-col mx-auto w-[275px] sm:w-60 rounded-xl hover:scale-110 transition-transform duration-500 ease-in group bg-gray-800 cursor-pointer">
        ...
        <div className="flex justify-center mt-3">
          <h1
            className="font-bold text-lg text-pink-100"
        ...
        <div className="flex justify-between items-center p-3">
          {cart.some((p) => p._id === item._id) ? (
            ...
            <button
              className="text-pink-500 font-semibold text-sm border-2 border-pink-500 p-2 rounded-lg  group-hover:bg-pink-500 group-hover:text-white transition-transform duration-300 ease-in-out group-hover:border-white"
        ...
        <div className="h-full w-full md:h-[600px] md:w-[450px] border-none rounded-lg outline-none bg-gray-700 absolute top-1/2 right-1/2 translate-x-1/2 -translate-y-1/2">
          <div className="flex flex-col items-center relative justify-center h-full">
            <div className="flex flex-col items-center justify-center space-y-5">
              ...
              <h1 className="text-xl text-pink-500 font-bold">{item.name}</h1>
              <p className="p-3 text-pink-100 font-semibold">
                {item.description}
              </p>
              <div className="flex items-center justify-between p-5 w-full">
              <p className="text-lg text-pink-100 font-bold">
                  Rs.{item.cost}
                </p>
               
                <button className="bg-pink-500 px-3 text-pink-100 font-bold text-lg hover:bg-pink-100 hover:text-pink-500 transition duration-300 ease-in p-3 rounded-xl">
                  Order now
                </button>
